...
if st.session_state.get("logout_button") == True:
    print('sitch page to front from admin panel')
    
    st.switch_page('pages/login.py')

def translate_text(text):
    """Перевод с использованием deep-translator"""
    try:
...
def click_button():
    """Отслеживание нажатия отправки фабулы в систему"""
...
if check_admin(st.session_state) == True:
    ...
    if user_input:
        ...
        with st.chat_message("assistant", avatar=":material/android:"):
            ...
            temp_message.write("🔨 Работаю над запросом. Это займет несколько минут. Ваш запрос сохранится во вкладке «Результаты»")    
